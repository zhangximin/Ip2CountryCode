...
int socket_start(char *PortNo) {
	int sfd, s;
	int efd;
	struct epoll_event event;
	struct epoll_event *events;
	int shutdown = 0;
//	char PortNo[] = SOCKET_PORT;
		...
		for (i = 0; i < n; i++) {
			if ((events[i].events & EPOLLERR) || (events[i].events & EPOLLHUP) || (!(events[i].events & EPOLLIN))) {
				...
				while (1) {
					...
					if (!strncmp(buf, "!reload", 7)){
						destroyJudyData();
						buildJudyData();
						s = write(events[i].data.fd, "DONE\n", 5);
						if (s == -1) {
							perror("write");
							abort();
						}
					...
					getCountryCode(buf, BufferSize);
					s = write(events[i].data.fd, buf, BufferSize);
					if (s == -1) {
						perror("write");
						abort();
					}
